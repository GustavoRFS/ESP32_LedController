...
void Api::registerRoutes(AsyncWebServer &server)
{
  ...
  server.on(
      "/api/connect-to-wifi", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL,
  ...
  server.on("/api/has-updates", HTTP_GET, [](AsyncWebServerRequest *request)
            {
    if (!UpdateService::checkForUpdate()) return sendResponse(200,"false",request);
    
    sendResponse(200,"true",request); });
